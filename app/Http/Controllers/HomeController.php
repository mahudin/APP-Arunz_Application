...
/*
        ...
        try {
            //$status = $client->checkCard($card_params);//sprawdzenie typu karty (czy prawidłowe)
            // $status = $client->generateToken($card_params3);

            $id_authorization = $client->cardAuthorization($card_params3);
            $resale_params = array(
            ...
            if ($client->isSuccess()) {
                ...
                echo "Success, id_authorization: ".join(",",$status)." \n";
            } else {
    ...
    public function get_past_roads()
    {
        ...
        return Json::encode($roads_for_ee);//$roads[0]->title." ".$roads[1]->title;
    }
