...
class User extends Authenticatable
{
    ...
    public static function is_after_attention_more_information($email)
    {
        $dane=DB::select('SELECT is_online FROM users where "email"=:email ',['email'=>$email]);
        return isset($dane[0]->is_online)? 1:0;
    }

    public static function check_by_login($array)
    {
	$dane=DB::select('SELECT id FROM users where "email"=:login LIMIT 1',
		['login'=>$array]);
	return $dane ? 1:0;
    }

    public static function authenticate_user($array)
    {
    ...
    public static function update_profile_user($dane,$id){
    ...
    public static function update_profile_additional($dane,$id){
        DB::update("UPDATE users
